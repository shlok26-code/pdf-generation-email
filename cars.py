...
def process_data(data):
  ...
  sorted_data = sorted(data, key=lambda x: x["total_sales"], reverse=False)
...
def cars_dict_to_table(car_data):
  ...
  for item in car_data:
    table_data.append([
        item["id"], 
        format_car(item["car"]), 
        float(item["price"].strip("$")),  # Ensure price is a float
        int(item["total_sales"])  # Ensure total_sales is an integer
    ])
  return table_data
...
def main(argv):
  ...
  summary, sorted_data = process_data(data)
  ...
  reports1.generate(attachment, title, paragraph, table_data)
